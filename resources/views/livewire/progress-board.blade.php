<x-ui-page>
    <x-slot name="navbar">
        <x-ui-page-navbar :title="$progressBoard->name" icon="heroicon-o-clock">
            <x-slot name="actions">
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        @can('update', $progressBoard)
                            ...
                            <x-ui-button variant="secondary-outline" size="sm" x-data @click="$dispatch('open-modal-progress-board-settings', { progressBoardId: {{ $progressBoard->id }} })" class="w-full">
                                <span class="inline-flex items-center gap-2">
                ...
                <div>
                    ...
                    <div class="space-y-1.5">
                        <div class="flex justify-between items-center py-1.5 px-3 bg-[var(--ui-muted-5)] border border-[var(--ui-border)]/40 rounded-lg text-sm">
                            ...
                            <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 border border-purple-200">
                                Progress
                            </span>
                        </div>
                        <div class="flex justify-between items-center py-1.5 px-3 bg-[var(--ui-muted-5)] border border-[var(--ui-border)]/40 rounded-lg text-sm">
                            <span class="text-[var(--ui-muted)]">Created</span>
                            <span class="text-[var(--ui-secondary)] font-medium">{{ $progressBoard->created_at->format('d.m.Y') }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex items-center justify-center h-full">
            <div class="bg-white rounded-xl border border-[var(--ui-border)]/60 shadow-sm p-12 text-center max-w-md">
                <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-purple-50 mb-4">
                    @svg('heroicon-o-clock', 'w-8 h-8 text-purple-600')
                </div>
